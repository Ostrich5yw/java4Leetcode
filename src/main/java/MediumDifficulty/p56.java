...
public class p56 {
    ...
    public int[][] merge(int[][] intervals) {
        ...
        while(startI < intervals.length - 1){
            for(int j = startI + 1;j < intervals.length;j ++){
                if(intervals[startI][1] < intervals[j][0]){
                    intervals[resLen][0] = intervals[startI][0];
                    intervals[resLen][1] = intervals[startI][1];
                    startI = j;
                    resLen ++;
                    ...
                    break;
                }else if(intervals[startI][1] >= intervals[j][0]){
                    intervals[startI][1] = Math.max(intervals[startI][1], intervals[j][1]);
                    if(j == intervals.length - 1){
                        intervals[resLen][0] = intervals[startI][0];
                        intervals[resLen][1] = intervals[startI][1];
                        startI = j;
        ...
        int[][] res = new int[resLen + 1][2];
        for(int x = 0; x <= resLen; x ++) {
            res[x][0] = intervals[x][0];
            res[x][1] = intervals[x][1];
        }
        return res;
    }
